...
```
Note: It is always preferable to create the table manually if it does not exist with a CREATE TABLE statement before executing the ```.import``` command


## Example
- Download the timesData.csv file from the [World University Rankings dataset](https://www.kaggle.com/datasets/mylesoneill/world-university-rankings) in Kaggle; create a new SQLite database and import the CSV file to a new temporary table; examine the imported data (e.g. data type, format, unit etc)
